<?php
    ...
    if (isInputEmpty($username, $password, $firstName, $lastName) !== false) {
        $dbc->close();
        header("location: ../WebClient/index.php?error=emptyInput#manage");
        exit();
    }
    if (isUsernameInvalid($username) !== false) {
        $dbc->close();
        header("location: ../WebClient/index.php?error=InvalidUsername#manage");
        exit();
    }
    if (isNameValid($firstName, $lastName) !== false) {
        $dbc->close();
        header("location: ../WebClient/index.php?error=invalidName#manage");
        exit();
    }
    if (usernameExists($dbc, $username)) {
        $dbc->close();
        header("location: ../WebClient/index.php?error=usernameTaken#manage");
        exit();
    }
    ...
    createUser($dbc, $username, $password, $firstName, $lastName);
        ...
        $dbc->close();
        header("location: ../WebClient/index.php#manage");
        exit();
    }
...
function createUser($dbc, $username, $password, $firstName, $lastName)
{
    ...
    header("location: ../WebClient/index.php?error=none#manage");
